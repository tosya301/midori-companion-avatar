...
const PendoloClockworkCanvas: React.FC<PendoloClockworkCanvasProps> = ({
    ...
    useEffect(() => {
        ...
        // Midori owns Day/Night outside this iframe; keep standalone Folia unchanged.
        let midoriHostRoot: HTMLElement | null = null;
        try {
            if (document.documentElement.classList.contains('lyric-stage-embed') && window.parent !== window) {
                midoriHostRoot = window.parent.document.documentElement;
            }
        } catch {
            // Cross-origin embeds retain the original gradient setting.
        }
        let animationFrameId: number;

        const render = (timestamp: number) => {
            const p = propsRef.current;
            const isMidoriDaylight = midoriHostRoot?.dataset.theme === 'light';
            // A muted grey-green keeps Day clockwork legible without recoloring lyrics.
            const gearPrimaryColor = isMidoriDaylight ? '#5F7D6B' : p.primaryTextColor;
            const gearAccentColor = isMidoriDaylight ? '#5F7D6B' : p.accentTextColor;
            ...
            // Read the live host theme so toggling does not reload or reset the clockwork.
            // 0. Optional Central Dark Radial Gradient using theme background color
            if (p.showCenterGradient && !isMidoriDaylight) {
            ...
            const primaryAlpha10 = colorWithAlpha(gearPrimaryColor, 0.10 * decorOpacityMultiplier);
            const primaryAlpha15 = colorWithAlpha(gearPrimaryColor, 0.15 * decorOpacityMultiplier);
            const primaryAlpha25 = colorWithAlpha(gearPrimaryColor, 0.25 * decorOpacityMultiplier);
            const accentAlpha12 = colorWithAlpha(gearAccentColor, 0.12 * decorOpacityMultiplier);
            const accentAlpha20 = colorWithAlpha(gearAccentColor, 0.20 * decorOpacityMultiplier);
            const accentAlpha35 = colorWithAlpha(gearAccentColor, 0.35 * decorOpacityMultiplier);
            const accentAlpha50 = colorWithAlpha(gearAccentColor, 0.50 * decorOpacityMultiplier);
            // Keep lyric guide rings/ticks subtle; lift only the mechanical assembly above them.
            const gearPrimaryAlpha = colorWithAlpha(gearPrimaryColor, 0.42 * decorOpacityMultiplier);
            const gearPrimarySubtleAlpha = colorWithAlpha(gearPrimaryColor, 0.32 * decorOpacityMultiplier);
            const gearAccentAlpha = colorWithAlpha(gearAccentColor, 0.58 * decorOpacityMultiplier);
            const gearAccentStrongAlpha = colorWithAlpha(gearAccentColor, 0.72 * decorOpacityMultiplier);
            const planetGearAlpha = colorWithAlpha(gearPrimaryColor, 0.24 * decorOpacityMultiplier);
            const jewelFillColor = colorWithAlpha(gearAccentColor, 0.28 * decorOpacityMultiplier);
            const jewelStrokeColor = colorWithAlpha(gearAccentColor, 0.65 * decorOpacityMultiplier);
            ...
            drawGearTeeth(
                ...
                gearAccentAlpha,
                2.2,
                colorWithAlpha(gearAccentColor, 0.08 * decorOpacityMultiplier),
            );
            ...
            ctx.strokeStyle = colorWithAlpha(gearPrimaryColor, 0.18 * decorOpacityMultiplier);
            ctx.lineWidth = 0.8;
            ctx.beginPath();
            ...
            for (let i = 0; i < guillocheRayCount; i++) {
                ...
                ctx.strokeStyle = i % 2 === 0 ? primaryAlpha10 : colorWithAlpha(gearPrimaryColor, 0.07 * decorOpacityMultiplier);
                ctx.lineWidth = 0.7;
                ctx.beginPath();
            ...
            for (let i = 0; i < rivetCount; i++) {
                ...
                ctx.strokeStyle = colorWithAlpha(gearPrimaryColor, 0.25 * decorOpacityMultiplier);
                ctx.lineWidth = 0.8;
                ctx.beginPath();
            ...
            drawGearTeeth(
                ...
                gearAccentStrongAlpha,
                2.1,
                colorWithAlpha(gearAccentColor, 0.10 * decorOpacityMultiplier),
            );
            ...
            drawHairspring(
                ...
                colorWithAlpha(gearAccentColor, 0.30 * decorOpacityMultiplier),
                0.8,
            );
            ...
            for (let i = 1; i <= genevaStripeCount; i++) {
                ...
                ctx.strokeStyle = i % 2 === 0 ? primaryAlpha10 : colorWithAlpha(gearPrimaryColor, 0.06 * decorOpacityMultiplier);
                ctx.lineWidth = 0.7;
                ctx.beginPath();
            ...
            drawGearTeeth(
                ...
                gearAccentAlpha,
                1.8,
                colorWithAlpha(gearAccentColor, 0.06 * decorOpacityMultiplier),
            );
            drawSpokedWheel(
                ctx,
                secondGearCx,
            ...
            drawGearTeeth(
                ...
                colorWithAlpha(gearPrimaryColor, 0.28 * decorOpacityMultiplier),
                1.3,
            );
            // Idler inner ring
            ctx.strokeStyle = colorWithAlpha(gearPrimaryColor, 0.18 * decorOpacityMultiplier);
