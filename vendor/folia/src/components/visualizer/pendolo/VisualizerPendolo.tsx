...
const VisualizerPendolo: React.FC<VisualizerSharedProps> = (props) => {
    ...
    useEffect(() => {
        ...
        if (!element || !document.documentElement.classList.contains('lyric-stage-embed')) return;
        ...
        // Midori Day: grey-green context lyrics with the Original/Luminous shadow, no outline.
        const syncContextStyle = () => {
            if (hostRoot.dataset.theme === 'light') {
                element.style.setProperty('--midori-pendolo-context-shadow', '0 2px 24px rgba(0, 0, 0, .38)');
                element.style.setProperty('--midori-pendolo-context-color', '#5F7D6B');
            } else {
                element.style.removeProperty('--midori-pendolo-context-shadow');
                element.style.removeProperty('--midori-pendolo-context-color');
            }
        };
        syncContextStyle();
        const observer = new MutationObserver(syncContextStyle);
                                    ...
                                    <div style={{ transform: 'translateY(-50%)' }}>
                                            ...
                                            <div>
                                                {isFocal ? (
                                                    ...
                                                    <div
                                                        className="pendolo-context-lyric transition-all duration-200 whitespace-pre-wrap"
                                                        style={{
                                                            textShadow: 'var(--midori-pendolo-context-shadow, inherit)',
                                                            fontSize: `${fontPx}px`,
                                                            maxWidth: `${maxTextWidth}px`,
                                                            color: `var(--midori-pendolo-context-color, ${colorWithAlpha(primaryTextColor, 0.75)})`,
