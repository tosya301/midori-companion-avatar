...
interface MeasureMonetLineLayoutOptions {
    line: Line;
    // [lyric-stage patch] status intentionally unused for measurement: reserving
    // the same box for waiting/active/passed stops the rail relayouting on every
    // line change. Kept in the signature so call sites stay unchanged.
    status?: MonetLineStatus;
    fontPx: number;
...
export const measureMonetLineLayout = ({
    ...
    const visibleTextLineCount = Math.min(textLineCount, MONET_ACTIVE_TEXT_LINE_LIMIT);
    const hasTranslation = showSubtitleTranslation && Boolean(line.translation?.trim());
    const rawTranslationLineCount = hasTranslation
