...
import type { CappellaConversationPack, CappellaConversationAvatar } from './conversationPack';
import { getMidoriCappellaPack } from './midoriConversationPack';
...
type VisualizerCappellaProps = VisualizerSharedProps & {
    conversationPack?: CappellaConversationPack;
};
...
const CappellaAvatar: React.FC<{
    ...
    conversationAvatar?: CappellaConversationAvatar;
}> = ({ avatarUrl, avatarIndex, theme, side, useAvatarGridCrop, conversationAvatar }) => {
    ...
    return (
        <motion.div
            ...
            role={conversationAvatar ? 'img' : undefined}
            aria-label={conversationAvatar?.name}
            title={conversationAvatar?.name}
            data-conversation-side={conversationAvatar ? side : undefined}
            style={{
                borderColor: 'rgba(255,255,255,0.24)',
                backgroundColor: conversationAvatar?.backgroundColor ?? theme.secondaryColor,
                ...
                backgroundSize: conversationAvatar?.backgroundSize ?? (shouldUseAvatarGridCrop ? avatarPosition.backgroundSize : 'cover'),
            }}
        />
    );
};
...
const CappellaMessageRow = React.forwardRef<HTMLDivElement, CappellaMessageRowProps>(({
    ...
    const conversationAvatar = conversationPack?.[message.side];
    const avatarUrl = conversationAvatar?.url ?? resolveCappellaAvatarUrl({
    ...
    const useAvatarGridCrop = !conversationAvatar && cappellaTuning.avatarSource === 'cover' && Boolean(coverUrl);
...
const VisualizerCappella: React.FC<VisualizerCappellaProps> = (props) => {
    const {
        ...
        isPreviewMode = false,
        conversationPack = getMidoriCappellaPack(),
    } = props;
    ...
    const resolvedCappellaTuning = useMemo<CappellaTuning>(() => ({
        showEmoMessages: conversationPack ? true : (cappellaTuning.showEmoMessages ?? DEFAULT_CAPPELLA_TUNING.showEmoMessages),
        emojiPackSource: (
    ...
    }), [conversationPack, cappellaCustomEmojiImages.length, cappellaCustomAvatarImages.length, cappellaTuning.avatarSource, cappellaTuning.emojiPackSource, cappellaTuning.showEmoMessages]);
    const activeEmoImages = useMemo(
        () => conversationPack ? conversationPack.stickers : resolvedCappellaTuning.emojiPackSource === 'custom' && cappellaCustomEmojiImages.length > 0
            ? cappellaCustomEmojiImages
            : builtinEmoImages,
        [conversationPack, cappellaCustomEmojiImages, resolvedCappellaTuning.emojiPackSource]
    );
