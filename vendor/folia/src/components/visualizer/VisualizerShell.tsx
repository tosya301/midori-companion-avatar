...
const VisualizerShell = forwardRef<HTMLDivElement, VisualizerShellProps>(({
    ...
    // [lyric-stage patch] In the hub embed the STAGE MENU pill covers the
    // top-left corner, so the hover-revealed back button flips to top-right.
    ...
    return (
        <div
            ...
            onMouseMove={(event) => {
                ...
                const nearBackArea = isLyricStageEmbed
                    ? event.clientX >= window.innerWidth - PLAYER_CHROME_HOTSPOT_SIZE && event.clientY <= PLAYER_CHROME_HOTSPOT_SIZE
            ...
            {resolvedOnBack && (
                <motion.button
                    ...
                    animate={{
                        ...
                        x: isBackButtonVisible ? 0 : (isLyricStageEmbed ? 6 : -6),
                    }}
                    ...
                    className={`absolute top-6 ${isLyricStageEmbed ? 'right-6' : 'left-6'} z-30 h-10 w-10 rounded-full flex items-center justify-center transition-colors backdrop-blur-md bg-black/20 hover:bg-white/10 text-white/60 pointer-events-auto`}
