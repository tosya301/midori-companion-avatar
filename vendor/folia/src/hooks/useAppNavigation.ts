...
export function useAppNavigation() {
    // [lyric-stage patch] Resolve the startup view on the very first render.
    // Leaving this as 'home' rendered the library/sign-in screen for one frame
    // before the effect below switched to the player, which flashed over the
    // stage on every load.
    const [currentView, setCurrentView] = useState<ViewState>(() => (
        typeof localStorage !== 'undefined' && localStorage.getItem(OPEN_PLAYER_ON_LAUNCH_KEY) === 'true'
            ? 'player'
            : 'home'
    ));
