...
// [lyric-stage patch] Helpers for upgrading Now Playing lyrics to word level.
// Line-level LRC still comes back with a populated words[] because the parser
// synthesises evenly-spread timings (buildTimedWords) — that is exactly the
// "moves but does not follow the singer" case. Only `isWordByWord`, which just
// the real karaoke sources set (QRC / YRC / KRC / TTML word timing), tells the
// two apart; autoMatchBestLyric itself uses the same signal.
const hasWordLevelTiming = (lyricData: LyricData | null | undefined): boolean => (
    Boolean(lyricData?.lines?.length && lyricData.isWordByWord)
);

const areAlternativeLyricSourcesEnabled = () => {
    if (typeof localStorage === 'undefined') return true;
    return localStorage.getItem('enable_alternative_lyric_sources') !== 'false';
};
...
export function useStagePlaybackController({
    ...
    const loadNowPlayingIntoPlayback = useCallback(async (
        ...
        // [lyric-stage patch] The Now Playing bridge only carries line-level
        // LRC, so words[] is synthesised with evenly-spread timings and every
        // line lights up at once. Ask folia's own matcher (netease → amll →
        // qq → kugou) for a karaoke version and swap it in only if it really
        ...
        if (!isLocalAuditionEmbed() && matchTitle && renderableLyrics && !hasWordLevelTiming(renderableLyrics) && areAlternativeLyricSourcesEnabled()) {
            ...
            void autoMatchBestLyric(
                ...
                Math.round(resolvedDurationSec * 1000),
                ...
                if (isDev) {
                    console.log('[NowPlaying][App] Upgraded to word-level lyrics', {
                        source: match.source,
                        lines: match.lyrics.lines.length,
                    });
                }
