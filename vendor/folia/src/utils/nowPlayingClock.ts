...
const IS_LYRIC_STAGE_EMBED = typeof window !== 'undefined'
    && new URLSearchParams(window.location.search).get('lyricStage') === '1';

// [lyric-stage patch] Midori owns Spotify credentials and exposes only a
// display-safe clock anchor. Standalone Folia keeps its original companion API.
export const NOW_PLAYING_PROGRESS_QUERY_URL = IS_LYRIC_STAGE_EMBED
    ? '/api/spotify/progress'
    : 'http://localhost:9863/api/query/progress';
