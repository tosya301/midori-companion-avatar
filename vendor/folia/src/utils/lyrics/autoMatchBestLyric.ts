...
export interface AutoMatchBestLyricOptions {
    ...
    /**
     * [lyric-stage patch] Overrides AUTO_MATCH_MIN_SCORE. Callers whose track
     * metadata is systematically romanised (Spotify reports "Gigi Leung" where
     * the CJK lyric catalogs store 梁咏琪) lose artist/album points on matches
     * that are otherwise correct. The title/identity gate still applies, so
     * lowering only this number cannot admit a wrong-title candidate.
     */
    minScore?: number;
...
function selectBestCandidate(
    ...
    minScore: number = AUTO_MATCH_MIN_SCORE
): SongResult | null {
    ...
    if (best.details.score < minScore) {
        console.log(`[autoMatchBestLyric] Skipping ${source} candidate because score ${best.details.score} is below ${minScore}`);
        return null;
    }
...
export async function autoMatchBestLyric(
    ...
    const getNeteaseCandidateSongs = async (): Promise<SongResult[]> => {
        ...
        const bestCandidate = selectBestCandidate('netease', neteaseSongs, targetSong, options.minScore);
    ...
    const getQqBestCandidate = async (): Promise<SongResult | null> => {
        ...
        qqBestCandidate = selectBestCandidate('qq', qqSongs, targetSong, options.minScore);
        return qqBestCandidate;
    };
    ...
    const getKugouBestCandidate = async (): Promise<SongResult | null> => {
        ...
        kugouBestCandidate = selectBestCandidate('kugou', page.items, targetSong, options.minScore);
        return kugouBestCandidate;
    };
