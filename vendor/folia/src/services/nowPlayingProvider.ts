...
export class NowPlayingProvider {
    ...
    private readonly usesParentTransport = typeof window !== 'undefined'
        && window.parent !== window
        && new URLSearchParams(window.location.search).get('lyricStage') === '1';
    private readonly handleParentMessage = (message: MessageEvent) => {
        if (message.source !== window.parent || message.origin !== window.location.origin) {
            return;
        }
        const data = message.data as { type?: string; events?: NowPlayingWsEnvelope[] } | null;
        if (data?.type !== 'midori-lyrics-events' || !Array.isArray(data.events)) {
            return;
        }
        data.events.forEach(event => this.handleMessage(JSON.stringify(event)));
    };
    ...
    start() {
        ...
        if (this.usesParentTransport) {
            window.addEventListener('message', this.handleParentMessage);
            this.updateConnectionStatus('connected');
            window.parent.postMessage({ type: 'midori-lyrics-ready', engine: 'folia' }, window.location.origin);
            return;
        }
        this.openSocket();
    }
