...
export default async function viteConfig(_config: ConfigEnv): Promise<UserConfig> {
  const isMidoriLyricStageBuild = process.env.MIDORI_LYRIC_STAGE_BUILD === '1';
  // Midori source distribution: deterministic labels, no enclosing Git history
  // lookup, no private branch/CI metadata, and no commit-name network request.
  // Public upstream provenance is recorded in docs/FOLIA_SOURCE_MANIFEST.json.
  const commitHash = 'source-distribution';
  const gitBranch = 'source-distribution';
  const commitSuffix = '';
  const appVersionLabel = 'Midori';
  const appReleaseChannel = 'source';
  const dockerStackVersion = '';
  ...
  return {
    ...
    plugins: [
      ...
      react(),
      VitePWA({
        disable: isMidoriLyricStageBuild,
