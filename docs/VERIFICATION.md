...
- 公共运行时45项Python、37项Node通过；沿用E9队列，更新latest-three运行素材及适配器。
- 实际公共页面通过完整8/8/6秒保持、14.4秒困倦循环的完整闭眼/睁眼、真实WAV口型且不唤醒、鼠标重置、七模式切换/销毁、加载失败/重试/竞态、减少动态与夜间窄屏检查；唯一宿主音频源，无页面异常或重复ID。
- 普通/近距/退出构图与更新前一致；维护者已人工验收表情。不是所有Windows硬件持续帧率保证。
- `tests/browser/test_latest_three.cjs`为当前入口；`test_e9_expressions.cjs`兼容转调同一套测试。生产模块无测试全局变量；只加速微笑间等待与闲置到期，完整保持/困倦动作实际计时，不调用真实Agent/云端TTS/私人账号。
- 发布前由审核提交打包，逐成员hash验证，在独立解压目录/干净HOME复跑运行时、公共浏览器、表情和Folia对应源码检查；附件digest核对通过后再发布。旧版本和附件保留。


## 0.2.0 常服表情更新

- 公共运行时 Python 45 项、Node 37 项测试通过；新增21项E9队列/适配器检查。
- 实际页面与本地WAV验证：三次普通微笑和wink顺序，圆口8秒/8秒、眯眼笑6秒的完整保持，犯困中语音不唤醒，鼠标唤醒重置，暂停/缓冲/恢复、单音频源、切装释放/失败重试/竞态、夜间窄屏。
- 原普通/近距/退出构图保持校准。维护者已人工确认本地公共版视觉无问题；不将其推论为所有设备满帧。
- 浏览器测试只跳过普通微笑间等待或推进闲置时钟，不压缩特殊表情保持；180000ms阈值由确定性测试验证，不冒充浏览器实等3分钟。无真实Agent/云端TTS/私人账号调用。
- 发布包只包含审核提交的公共树，逐成员hash核对，并在独立解压目录/干净HOME执行运行时与浏览器复验；附件核对通过后才发布。
- 新入口为`tests/browser/test_e9_expressions.cjs`；设置`MIDORI_TEST_OUTPUT`、`MIDORI_TEST_WAV`（调用者有权使用的真实本地语音文件），其他浏览器变量同下文。测试插桩仅存在于拦截响应，生产模块没有全局测试接口。

以下0.1记录保留作历史基线，不代表此次重跑所有历史检查。
