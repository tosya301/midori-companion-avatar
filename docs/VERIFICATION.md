...
- Python：45 项通过；Node：16 项通过，包含音乐试听提示记忆与番茄钟模板完整性回归。
- 新语音03已实际播放；新Agent语音成功后自动收起、调试重开、刷新及origin隔离已通过浏览器检查。事件为明确标注的测试夹具，不是真实Agent/TTS调用。
- 本地歌曲实际播放，49句时间轴、歌词舞台、暂停/拖动/停止已检查；提示在播放成功、明确不再提示或收到连接状态后记忆，取消/失败不误触发，独立手动入口可重开。音乐服务状态使用测试夹具，不冒充账号实连。
- 番茄钟缺失HTML模板已补齐：首次面板可见，收起后刷新仍有右上角恢复图标，桌面/窄屏、开始/暂停/重置通过。此前仅检查页面异常未捕捉到已catch的初始化报错，现加入模板存在性、相关console.error及HTTP失败检查。
- 泳装使用已验收的重制SVG；不宣称所有Windows设备满帧。
- 最终ZIP由已提交公共树生成，逐成员CRC/文件数/哈希验证；发布操作另对解压副本执行干净HOME运行与浏览器检查，GitHub附件digest与本地文件匹配后才发布。

以下为前序公开预览阶段的历史基线，旧测试数及截图不能代替上述增量检查。
...
- 当时的增补检查：45 Python + 12 Node 测试通过；浏览器实播全部样音（含新 MP3），验证新 Agent 语音成功后的自动收起/重新展开/刷新持久化，以及未知来源、音乐、测试与历史重播不触发。使用合成 Agent 事件夹具，不调用真实 Agent/TTS。公开版 5178 本地曲目播放、49 条歌词、拖动/暂停/继续/退出通过，零页面异常。
- 原发布范围：三条内置音频均真实播放；两条人声样音的名称、选择、播放时钟和 RMS 已检查，提示音的口型响应已检查。样音试听不调用 TTS。
...
不包含私人 Git 历史、参考录音、克隆模型、朗读历史、账户配置或生成音频缓存。原发布时随附人声为两条维护者确认纳入的测试样音；0.1新增人声03与音乐试听，见上述增补及 ASSET_RIGHTS.md；图片/角色媒体按维护者本次授权确认保留。素材许可不覆盖未来新增素材，也不免除第三方代码许可义务，详见 `ASSET_RIGHTS.md` 和 `LICENSE_SCOPE.md`。
